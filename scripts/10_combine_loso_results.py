...
Generates summary tables with F1, MCC, Precision, and Recall for all species
UPDATED: Extracts F1, MCC, Precision, and Recall from fold_metrics.npy for ALL models
    ...
    if os.path.exists(npy_path):
        try:
            ...
            if 'test_fixed' in fold_metrics:
                ...
                # Extract all metrics with mean and std
                ...
                precision_vals = test_metrics.get('precision', [np.nan])
                recall_vals = test_metrics.get('recall', [np.nan])
                
                # Handle case where precision/recall might be empty or all NaN
                if precision_vals and not all(np.isnan(v) for v in precision_vals):
                    result['test_precision'] = np.mean(precision_vals)
                    result['test_precision_std'] = np.std(precision_vals)
                else:
                    result['test_precision'] = np.nan
                    result['test_precision_std'] = np.nan
                
                if recall_vals and not all(np.isnan(v) for v in recall_vals):
                    result['test_recall'] = np.mean(recall_vals)
                    result['test_recall_std'] = np.std(recall_vals)
                else:
                    result['test_recall'] = np.nan
                    result['test_recall_std'] = np.nan
...
def format_metric(value, std):
    ...
    if value is None or pd.isna(value):
        return 'N/A'
    if std is None or pd.isna(std) or np.isnan(std):
        return f"{value:.4f}"
...
def main():
        ...
        for method in method_list:
            ...
            if best:
                ...
                result = {
                    ...
                    'test_precision_std': metrics.get('test_precision_std', np.nan),
                    'test_recall': metrics.get('test_recall', np.nan),
                    'test_recall_std': metrics.get('test_recall_std', np.nan),
    ...
    table1_output.columns = ['Species', 'Best Method', 'Accuracy', 'AUC', 'F1', 'MCC', 'Precision', 'Recall']
    
    print("\n" + table1_output.to_string(index=False))
    
    # Save
    table1_output.to_csv(os.path.join(OUTPUT_DIR, 'Table1_LOSO_Best_Per_Species_Updated.csv'), index=False)
    ...
    # Save
    table2_output.to_csv(os.path.join(OUTPUT_DIR, 'Table2_LOSO_Avg_Per_Method_Updated.csv'))
    ...
    # Save
    paper_summary.to_csv(os.path.join(OUTPUT_DIR, 'Table3_LOSO_Paper_Summary_Updated.csv'), index=False)
    ...
    display_avg['MCC'] = display_avg.apply(
        lambda x: format_metric(x['test_mcc_mean'], x['test_mcc_std']), axis=1)
    display_avg['Precision'] = display_avg.apply(
        lambda x: format_metric(x['test_precision_mean'], x['test_precision_std']), axis=1)
    display_avg['Recall'] = display_avg.apply(
        lambda x: format_metric(x['test_recall_mean'], x['test_recall_std']), axis=1)
    
    table4_output = display_avg[['Accuracy', 'AUC', 'F1', 'MCC', 'Precision', 'Recall']]
    table4_output.index.name = 'Method'
    
    print("\n" + table4_output.to_string())
    
    # Save
    table4_output.to_csv(os.path.join(OUTPUT_DIR, 'Table4_LOSO_Average_All_Species_Updated.csv'))
    ...
    best_overall_df.to_csv(os.path.join(OUTPUT_DIR, 'Best_LOSO_Overall_Updated.csv'), index=False)
    ...
    summary_by_type.to_csv(os.path.join(OUTPUT_DIR, 'Table5_LOSO_Summary_By_Type_Updated.csv'))
    
    # =============================================
    # 8. Save Full Results
    # =============================================
    all_loso.to_csv(os.path.join(OUTPUT_DIR, 'all_loso_results_detailed_updated.csv'), index=False)
    
    print("\n" + "=" * 80)
    print("✅ ALL LOSO TABLES GENERATED WITH PRECISION AND RECALL")
    print("=" * 80)
    print(f"Output directory: {OUTPUT_DIR}")
    print("\nFiles generated:")
    print(f"  - Table1_LOSO_Best_Per_Species_Updated.csv")
    print(f"  - Table2_LOSO_Avg_Per_Method_Updated.csv")
    print(f"  - Table3_LOSO_Paper_Summary_Updated.csv")
    print(f"  - Table4_LOSO_Average_All_Species_Updated.csv")
    print(f"  - Table5_LOSO_Summary_By_Type_Updated.csv")
    print(f"  - Best_LOSO_Overall_Updated.csv")
    print(f"  - all_loso_results_detailed_updated.csv")
    print("=" * 80)
